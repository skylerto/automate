...
func (backend *ESClient) InsertInspecSummary(ctx context.Context, id string, endTime time.Time, data *relaxting.ESInSpecSummary) error {
	...
	if data.DayLatest {
		err = backend.setYesterdayLatestToFalse(ctx, data.NodeID, id, mapping)
		if err != nil {
			return err
		}
	}
...
func (backend *ESClient) setYesterdayLatestToFalse(ctx context.Context, nodeId string, reportId string, mapping mappings.Mapping) error {
	...
	// TODO: Refresh set to false prevented the setLatestsToFalse to work
